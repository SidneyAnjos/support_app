...
Live app: supportapps-7474659512156367.aws.databricksapps.com

Access is restricted to users with permission on the Databricks workspace. Databricks Apps do not support public/anonymous access — see Authentication below.
...
Database	Lakebase (managed Postgres, OLTP)
DB driver	psycopg2
...
Lakebase is added as a resource to the Databricks App, which injects PGHOST and PGUSER as environment variables automatically.
The app authenticates to Postgres using a short-lived OAuth token, generated at runtime via databricks.sdk.core.Config().oauth_token(), using the app's own service principal identity.
Database-level permissions are granted directly to the service principal (DATABRICKS_CLIENT_ID) via GRANT statements in the Lakebase SQL Editor — no static credentials are ever created or shared.
python
...
└── README.md
Deployment
Create a Lakebase project (branch + database) in your Databricks workspace.
Create the support_app schema and tables above (see tickets / ticket_messages DDL).
Create the Databricks App and connect it to your GitHub repository.
In the app's Resources tab, add the Lakebase project/branch/database as a resource — this injects PGHOST/PGUSER automatically.
In the Lakebase SQL Editor, grant the app's service principal (DATABRICKS_CLIENT_ID, found in the app's Environment tab) access to the schema:
sql
